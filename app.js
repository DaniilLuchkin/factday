...
async function init() { const response = await fetch('facts.json'); facts = await response.json(); renderCategories(); showFact(todayIndex()); renderSaved(); renderHistory(); setupInteractions(); if ('serviceWorker' in navigator) navigator.serviceWorker.register('sw.js').catch(() => {}); if (localStorage.getItem('factday-theme') === 'dark') setTheme(true); }
